// frontend/src/app/dashboard/groups/[id]/members/page.tsx
"use client";
...
export default function GroupMembersPage({
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      toast({
        title: "メンバーを追加しました",
        description: "グループメンバーが正常に追加されました",
      });
    } catch (error) {
  ...
  const handleRemoveMember = async (userId: number) => {
    const confirm = window.confirm("このメンバーをグループから削除しますか？");
    if (!confirm) return;

    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/groups/${params.id}/members/${userId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
