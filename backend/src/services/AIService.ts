...
export class AIService {
  ...
  private async analyzeCodeWithContext(
    ...
    const promptTemplate = PromptTemplate.fromTemplate(`
      あなたはエキスパートプログラマーとして、プルリクエスト内のコード変更をレビューする任務を負っています。
      以下のファイルの変更を徹底的に分析し、問題点や改善点を特定してください。
      ...
      2. コードの品質、可読性、保守性、セキュリティ、パフォーマンスの観点からレビューしてください
      3. 明確な問題が見つかった場合は、具体的な改善方法を提案してください
      4. Git Diffの行番号を参照して、問題のある箇所を具体的に示してください
      5. 優先度を適切に設定してください（high: 重大な問題、medium: 改善すべき問題、low: 小さな提案）

      結果は以下の形式で返してください：
      - コードに問題がある場合：問題点と改善提案を含むJSON配列
      - コードが十分に優れている場合：空の配列（[]）
  ...
  private async analyzeSubmissionCode(
    ...
    コード内のコメント、変数名、関数名、ドキュメンテーションに誤字脱字がないか確認すること。
    一般的なベストプラクティスに基づくレビュー

    コードの可読性、保守性、セキュリティ、パフォーマンス向上のための改善点を見出す。
    適切な命名規則、コードのモジュール化、コメントの書き方、エラーハンドリング、テストの有無など、一般的なベストプラクティス全般に即して評価すること。
    IT企業の新入社員向けの解説

    新入社員が理解しやすいよう、基本的な概念や背景、理由を丁寧に解説する。
    なぜその改善が必要か、またどのような手法が推奨されるか、具体例を用いて説明すること。
    以上のポイントを踏まえ、レビュー対象のコードに対して具体的なフィードバックと改善案を示すこと。
