// frontend/src/app/dashboard/admin/repositories/page.tsx
"use client";
...
export default function RepositoriesManagementPage() {
  ...
  const handleSyncRepositories = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/backlog/sync-repositories`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        throw new Error("リポジトリの同期に失敗しました");
      }

      toast({
        title: "同期完了",
        description: "Backlogからリポジトリ情報を同期しました",
      });
      ...
      if (refreshResponse.ok) {
        ...
        setRepositories(refreshData.data || []);
      }
    } catch (error) {
      console.error("リポジトリ同期エラー:", error);
      toast({
        title: "同期エラー",
        description: "Backlogからのリポジトリ同期に失敗しました",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">リポジトリ管理</h1>
        <p className="text-gray-500 mt-1">
          Backlogリポジトリの連携設定とホワイトリスト管理を行います
        </p>
      </div>
      ...
      >
        <TabsList className="grid w-full md:w-[500px] grid-cols-2">
          <TabsTrigger value="whitelist" className="flex items-center">
        ...
        <TabsContent value="whitelist" className="space-y-4 mt-4">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {repositories.map((repo) => (
                        <TableRow
                          key={`${repo.projectKey}-${repo.repositoryName}`}
                        >
          ...
          <Card>
            <CardHeader>
              <CardTitle>ホワイトリスト設定について</CardTitle>
            </CardHeader>
        ...
        <TabsContent value="settings" className="space-y-4 mt-4">
          <Card>
            <CardHeader>
              <CardTitle>リポジトリ接続設定</CardTitle>
              <CardDescription>
                Backlogリポジトリに関する全般的な設定を管理します
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                <div className="bg-muted p-4 rounded-md flex items-center justify-between">
                  <div>
                    <h3 className="font-medium">リポジトリ同期</h3>
                    <p className="text-sm text-gray-500 mt-1">
                      Backlogから最新のリポジトリ情報を取得します
                    </p>
                  </div>
                  <Button
                    variant="outline"
                ...
                <div className="bg-muted p-4 rounded-md">
                  <h3 className="font-medium">Backlog連携設定</h3>
                  <p className="text-sm text-gray-500 mt-1">
                    この機能は開発中です。今後のアップデートをお待ちください。
                  </p>
                </div>
              </div>
            </CardContent>
